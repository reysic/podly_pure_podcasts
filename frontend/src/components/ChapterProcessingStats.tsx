            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
              {isLoading ? (
                ...
                <>
                  {activeTab === 'overview' && (
                    <div className="space-y-6">
                      ...
                      {stats.chapters?.filter_strings && (
                        <div>
                          <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4 text-left">Filter Strings</h3>
                  ...
                  {activeTab === 'chapters' && (
                    <div>
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4 text-left">Chapters ({stats.chapters?.chapters?.length || 0})</h3>
                            ...
                            <tbody className="bg-white dark:bg-gray-750 divide-y divide-gray-200 dark:divide-gray-700">
                              {(stats.chapters?.chapters || []).map((chapter: { title: string; start_time: number; end_time: number; label: string }, idx: number) => (
                                <tr key={idx} className={`hover:bg-gray-50 dark:hover:bg-gray-800 ${
                                  chapter.label === 'ad' ? 'bg-red-50 dark:bg-red-950' : ''
                                }`}>
                                  <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{idx + 1}</td>
                                  <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 font-medium">{chapter.title}</td>
                      ...
                      {stats.chapters?.note && (
                        <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-700 rounded-lg text-sm text-amber-800 dark:text-amber-300">

...
def _get_post_timestamp(
    ...
    of when processing finished. File mtime is used as a fallback.
    """
    job_timestamp = latest_completed.get(post.guid)
    file_timestamp = _get_processed_file_timestamp(post)

    # Prefer job timestamp as the authoritative record of when processing completed
    return job_timestamp or file_timestamp
...
def cleanup_processed_posts(retention_days: int | None) -> int:
    ...
    Eligible posts are un-whitelisted and audio files are removed, but the
    post row and all processing metadata (jobs, segments, model calls) are
    retained for historical tracking. Returns the number of posts that were
    cleaned. Callers must ensure an application context is active.
    """
